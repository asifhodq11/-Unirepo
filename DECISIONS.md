...
Users who sign up on the 31st of a month will not receive their monthly reset in February, April, June, September, or November. Affects <1% of users. Acceptable for V1. Fix in V2 with `last_day_of_month()` logic.

### KL-02: Coverage Cap (59%)
The current test coverage is below the 70% project requirement. The system is prone to regression during refactors. This is a technical debt item for V1.1.

### KL-03: LLM Pattern Evolution (Stilted Output Risk)
The "De-Botting" pass uses a static list of 24 patterns. As models like GPT-4o evolve to be more natural, this list may start stripping valid human speech, potentially making-the output feel "stilted" or "over-corrected."

### KL-04: Supabase Lock-in
The backend logic is tightly coupled to Supabase-specific Python SDK features and RPC implementations, making migration to a generic Postgres provider or another BaaS (like Firebase) a non-trivial refactor.
