...
# The 33 AI Patterns to eliminate — hyper-aggressive anti-slop
THIRTY_THREE_PATTERNS = """
1. Em dashes (—)
2. Vibrant / pivotal
3. Foster / cultivate
4. Showcase / leverage
...
BANNED AI PATTERNS — Do not use ANY of these under ANY circumstances:
{THIRTY_THREE_PATTERNS.strip()}
"""
...
def _build_pass2_prompts(pass1_output: str, review_text: str, star_rating: int) -> tuple[str, str]:
    ...
    system_prompt = """You are a ruthless Slop-Detecting Filter. Your sole purpose is to destroy every cliche, corporate phrase, and generic 'polite' AI-ism from this draft.
If a sentence sounds like it was generated by a machine or a corporate PR team, you MUST rewrite it to be blunt, authentic, and human.
OUTPUT: The revised reply ONLY. No commentary. No preamble."""
...
□ Opener check: Does it start with "Thank you" or the reviewer's name?
  If yes → rewrite the opener entirely. Start from their experience, not gratefulness.
□ Closer check: Does it end with "hope to see you soon" / "look forward to welcoming you"?
  If yes → replace with something highly specific.

WORD-LEVEL FIXES (ZERO TOLERANCE):
□ Strip EVERY word from this list: wonderful, amazing, delighted, thrilled, ensure,
  ...
  absolutely, certainly, definitely, truly, genuinely, glad to hear, appreciate your feedback.
□ Replace em dashes (—) completely.
□ Remove hollow emphasis entirely.
...
□ Introduce ONE deliberate human irregularity:
...
{amygdala_block}
SLOP REPLACEMENT TABLE (MANDATORY):
□ If you see "I'm/we're glad to hear" or "thrilled" → REPLACE with a direct observation about the item.
□ If you see "I/we understand" → REPLACE with "That makes sense" or remove it.
□ If you see "We appreciate" → REPLACE with a specific acknowledgment.

AI VOCABULARY FINAL CHECK:
□ Scan for: wonderful, amazing, ensure, experience (noun), journey,
  truly, definitely, certainly, glad to hear, appreciate, thrilled.
...
def generate_reply(
    ...
    if not has_text:
        ...
        sys_1 = (
            ...
            f"BANNED PATTERNS: {THIRTY_THREE_PATTERNS.replace(chr(10), ' ')}"
        )
        usr_1 = (
