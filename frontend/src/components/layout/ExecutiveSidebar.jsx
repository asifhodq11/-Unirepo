...
export const ExecutiveSidebar = () => {
  ...
  return (
    <aside className="w-64 h-screen flex flex-col bg-slate-950 border-r border-slate-800/60 sticky top-0 left-0 z-50">
      {/* Brand Header */}
      ...
      <nav className="flex-1 px-4 py-8 space-y-9 overflow-y-auto custom-scrollbar">
        ...
        {/* Core Tools */}
        <div>
          <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] pl-3 mb-4">Operations</h3>
              ...
              return (
                <li key={item.path}>
                  <NavLink to={item.path} className="relative block group">
                    <div className={`relative z-10 flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${isActive ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-500/20 shadow-sm' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900 border border-transparent'}`}>
        ...
        <div>
          <h3 className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.2em] pl-3 mb-4">Management</h3>
          <ul className="space-y-1.5">
            {SYS_ITEMS.map((item) => {
            ...
            {isAdmin && (
              <li>
                <NavLink to="/admin" className="relative block group">
                  <div className={`relative z-10 flex items-center gap-3 px-3 py-2 rounded-lg transition-all duration-200 ${location.pathname === '/admin' ? 'bg-indigo-600/10 text-indigo-400 border border-indigo-500/20' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-900 border border-transparent'}`}>
                    <ShieldCheck size={18} className={location.pathname === '/admin' ? 'text-indigo-400' : 'text-slate-500 group-hover:text-slate-300'} />
